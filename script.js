    ...
    modal.innerHTML = `
        <div class="modal-box">
            ...
            <div class="modal-actions">
                <button class="btn-confirm-send" onclick="window.open('${mailtoUrl}', '_blank'); confirmDelivery(this);">Confirm & Open Email App</button>
                <button class="btn-close-modal" onclick="this.closest('.modal-overlay').remove()">Cancel</button>
            </div>
        </div>
    `;
...
function renderSponsors() {
    ...
    htmlContent += `
        <div class="sponsor-card gold-badge" style="border-style: dashed; background: transparent; justify-content: center; width: 100%; max-width: 500px;">
            <div class="sponsor-info" style="text-align: center;">
                <span class="sponsor-name" style="color: #d97706; font-size: 0.95rem;">🏆 Want to be the next sponsor and show up here?</span>
                <span class="sponsor-status" style="color: #9ca3af; margin-top: 0.25rem;">If you are a company, make a donation of 0.005 BTC and add a memo/note with your transaction so we know WHO to award the official sponsor badge to!</span>
            </div>
        </div>
    `;
...
document.addEventListener('click', async function(e) {
    ...
    if (clickedButton && inputField) {
        ...
        let found = null;
        const trackingItems = document.querySelectorAll('.shame-list li');
        trackingItems.forEach(item => {
            const compNameEl = item.querySelector('.company-name');
            if (compNameEl && compNameEl.textContent.trim().toLowerCase() === query.toLowerCase()) {
                found = { name: compNameEl.textContent.trim() };
            }
        });
        ...
        if (liveResult) {
            if (found) {
                liveResult.innerHTML = `
                    <div class="result-card danger">
                        <h3>⚠️ Match Found</h3>
                        <p><strong>${found.name}</strong> is on this list because they do not follow privacy guidelines.</p>
                        <button class="btn-action-claim" id="btnLiveOptOut">Send Official Opt-Out Request</button>
                    </div>
                `;
                document.getElementById('btnLiveOptOut').addEventListener('click', async function() {
                ...
                liveResult.innerHTML = `
                    <div class="result-card success">
                        <h3>✅ Clear</h3>
                        <p><strong>${query}</strong> is not in our database (yet).</p>
                        <button class="btn-action-report" id="btnLiveReportTrigger">Report This Company</button>
                    </div>
                `;
